...
class StreamHandler
{
    ...
    private function createStream(RequestInterface $request, array $options)
    {
        ...
        return $this->createResource(function () use($uri, $contextResource, $context, $options, $request) {
            ...
            if (\function_exists('WpToTwitter_Vendor\\http_get_last_response_headers')) {
                $http_response_header = \WpToTwitter_Vendor\http_get_last_response_headers();
            }
    ...
    private static function triggerUnsupportedRequestOptionDeprecations(RequestInterface $request, array $options) : void
    {
        if (\array_key_exists('curl', $options) && $options['curl'] !== null && $options['curl'] !== [] && !self::isCurlOptionGeneratedByAuth($options)) {
            \WpToTwitter_Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "curl" request option to the stream handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because the stream handler ignores cURL options.');
        }
        if (\array_key_exists('expect', $options) && $options['expect'] !== \false && $request->hasHeader('Expect')) {
            \WpToTwitter_Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "expect" request option to the stream handler is deprecated when it adds an Expect header; guzzlehttp/guzzle 8.0 will reject this option because the stream handler does not support Expect: 100-Continue.');
        }
    }
