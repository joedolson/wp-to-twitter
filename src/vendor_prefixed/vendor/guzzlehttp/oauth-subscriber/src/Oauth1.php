...
class Oauth1
{
    ...
    public function getSignature(RequestInterface $request, array $params) : string
    {
        ...
        $baseString = $this->createBaseString($request, self::prepareParameters($params));
    ...
    protected function createBaseString(RequestInterface $request, array $params) : string
    {
        ...
        return \strtoupper($request->getMethod()) . '&' . \rawurlencode((string) $request->getUri()->withQuery('')) . '&' . \rawurlencode(Query::build($params));
    }
    /**
    ...
    private function signUsingRsaSha1(string $baseString) : string
    {
        ...
        if (!isset($this->config['private_key_file']) || !\is_string($this->config['private_key_file']) || $this->config['private_key_file'] === '') {
        ...
        if (isset($this->config['private_key_passphrase'])) {
            ...
            $privateKey = \openssl_pkey_get_private(\file_get_contents($this->config['private_key_file']));
        }
        $signature = '';
    ...
    {
        $params = ['oauth_consumer_key' => $config['consumer_key'], 'oauth_nonce' => \bin2hex(\random_bytes(20)), 'oauth_signature_method' => $config['signature_method'], 'oauth_timestamp' => \time()];
