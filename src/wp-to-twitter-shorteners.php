		...
		if ( $testmode == false ) {
			if ( get_option( 'use-twitter-analytics' ) == 1 || get_option( 'use_dynamic_analytics' ) == 1 ) {
				if ( get_option( 'use_dynamic_analytics' ) == '1' ) {
					...
					} elseif ( $campaign_type == "post_ID" ) {
						$campaign = $post_ID;
					} elseif ( $campaign_type == "post_title" && $testmode != 'link' ) {
			...
			<div class="postbox">
				...
				<div class="inside">
					...
					<?php } elseif ( $shortener == 2 ) { ?>
						...
						<?php echo $form_end; ?>
					<?php } elseif ( $shortener == 5 || $shortener == 6 ) { ?>
						<?php echo $form_start; ?>
						<?php if ( $shortener == 5 ) { ?>
						<p>
							<label
	...
	function wpt_shortener_update( $post ) {
		...
		if ( isset( $post['submit-type'] ) && $post['submit-type'] == 'yourlsapi' ) {
			...
			if ( $post['jd_keyword_format'] != '' ) {
				...
				} elseif ( $post['jd_keyword_format'] == 0 ) {
